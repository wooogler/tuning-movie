...
export interface BaselineRouterInput {
  ...
  availableToolNames: Array<'select' | 'selectMultiple' | 'next' | 'prev'>;
...
interface BaselineRouterAction {
  toolName: 'select' | 'selectMultiple' | 'next' | 'prev' | 'none';
...
const OPENAI_MODEL = process.env.AGENT_OPENAI_MODEL || 'gpt-5.4';
...
const BASELINE_ROUTER_SYSTEM_PROMPT =
  ...
  'Allowed action.toolName values: select, selectMultiple, next, prev, none.\n' +
  'Rules:\n' +
  ...
  '- Only choose next or prev when the user clearly expresses that navigation intent.\n' +
    ...
    properties: {
      action: {
        ...
        properties: {
          toolName: {
            type: 'string',
            enum: ['select', 'selectMultiple', 'next', 'prev', 'none'],
          },
          params: {
            type: 'object',
            properties: {
