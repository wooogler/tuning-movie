...
type BaselineToolName = 'select' | 'selectMultiple' | 'next' | 'prev';
...
function repeatStepDecision(fallbackReason: string, reason?: string): PlanDecision {
  return {
    action: toolCall(
      'repeatStep',
      {},
      reason ?? 'Repeat the current stage because the input did not map cleanly to one GUI action.'
    ),
    source: 'baseline',
    fallbackReason,
  };
...
function stageGoal(stage: Stage): string {
  switch (stage) {
    ...
    case 'time':
      return 'Pick one showtime with screen type.';
    case 'seat':
