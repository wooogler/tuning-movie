...
function extractUserMessage(entry: InteractionLogEntry): UserMessage | null {
  if (entry.type !== 'chat.message.rendered') {
    return null;
  }
  ...
  const message = isRecord(entry.payload.message) ? entry.payload.message : null;
  if (!message || message.type !== 'user' || message.action !== 'input') {
    return null;
  }

  const text = asNonEmptyString(message.label);
  if (!text) {
    return null;
  }
  ...
  return {
    ...
    stage: asNonEmptyString(message.stage),
    text,
  };
}
