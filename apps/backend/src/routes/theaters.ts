...
import { normalizeParkingAmenities } from '../utils/parkingAmenities';
...
function parseAmenities(value: string): string[] {
  try {
    ...
    if (Array.isArray(parsed)) {
      return normalizeParkingAmenities(
...
export async function theaterRoutes(fastify: FastifyInstance) {
  ...
  fastify.get('/theaters', async (request) => {
    ...
    const ordered = stableShuffleForSession(request, result, 'theaters:all', (theater) => theater.id);
    return { theaters: ordered.map(toTheaterResponse) };
  });
  ...
  fastify.get('/theaters/movie/:movieId', async (request) => {
    ...
    const ordered = stableShuffleForSession(
      request,
      availableTheaters,
