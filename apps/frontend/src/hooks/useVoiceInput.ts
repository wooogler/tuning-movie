...
const VOICE_IDLE_NOISE_FLOOR = 0.008;
const VOICE_IDLE_NOISE_ALPHA = 0.12;
const VOICE_START_THRESHOLD_MIN = 0.035;
const VOICE_START_THRESHOLD_MAX = 0.09;
const VOICE_START_THRESHOLD_NOISE_MARGIN = 0.02;
const VOICE_CONFIRM_THRESHOLD_OFFSET = 0.004;
const VOICE_START_HOLD_MS = 90;
const VOICE_FALSE_START_TIMEOUT_MS = 600;
...
function updateNoiseFloor(current: number, level: number): number {
  return current + (level - current) * VOICE_IDLE_NOISE_ALPHA;
}

function getVoiceStartThreshold(noiseFloor: number): number {
  return clamp(
    noiseFloor + VOICE_START_THRESHOLD_NOISE_MARGIN,
    VOICE_START_THRESHOLD_MIN,
    VOICE_START_THRESHOLD_MAX
  );
}
...
export function useVoiceInput({
  ...
  const noiseFloorRef = useRef(VOICE_IDLE_NOISE_FLOOR);
  ...
  useEffect(() => {
    ...
    const analyzeAudio = () => {
      ...
      if (suspendedRef.current) {
        ...
        const startThreshold = getVoiceStartThreshold(noiseFloor);
        const confirmThreshold = Math.max(
          VOICE_START_THRESHOLD_MIN - VOICE_CONFIRM_THRESHOLD_OFFSET,
          startThreshold - VOICE_CONFIRM_THRESHOLD_OFFSET
        );
        const recorder = recorderRef.current;

      if (recorder && recorder.state !== 'inactive') {
        if (level >= confirmThreshold) {
        ...
        if (!speechConfirmedRef.current) {
          ...
          if (level < confirmThreshold && now - candidateStartedAt >= VOICE_FALSE_START_TIMEOUT_MS) {
            stopRecorder(true);
          }
      ...
      if (level >= startThreshold) {
        ...
        noiseFloorRef.current = updateNoiseFloor(noiseFloorRef.current, level);
        speechCandidateStartedAtRef.current = null;
      }
