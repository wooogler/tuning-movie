...
const SCENARIO_TITLE_PATTERN = /^S(\d+)-T(\d+):\s*(.+?)\s*\((Easy|Hard)\)$/i;
const SCENARIO_ID_PATTERN = /^scn_s\d+_t([12])_/i;

export type ScenarioDifficulty = 'easy' | 'hard';
...
function toSetLabel(setNumber: number): string {
  if (setNumber >= 1 && setNumber <= 26) {
    return String.fromCharCode(64 + setNumber);
  }
  return String(setNumber);
}

export function getScenarioDifficulty({
  scenarioId,
  scenarioTitle,
}: {
  ...
  if (normalizedTitle) {
    const titleMatch = normalizedTitle.match(SCENARIO_TITLE_PATTERN);
    if (titleMatch) {
      return titleMatch[4].toLowerCase() as ScenarioDifficulty;
    }
  }

  const normalizedId = scenarioId?.trim();
  if (normalizedId) {
    const idMatch = normalizedId.match(SCENARIO_ID_PATTERN);
    if (idMatch) {
      return idMatch[1] === '2' ? 'hard' : 'easy';
    }
  }
...
export function isHardTaskScenario({
  scenarioId,
  scenarioTitle,
}: {
  scenarioId?: string | null;
  ...
  return getScenarioDifficulty({ scenarioId, scenarioTitle }) === 'hard';
}

export function formatScenarioTitle(value: string): string {
  const normalized = value.trim();
  if (!normalized) return value;

  const match = normalized.match(SCENARIO_TITLE_PATTERN);
  if (!match) return normalized;

  const setNumber = Number.parseInt(match[1], 10);
  const scenarioTheme = match[3].trim();
  const difficulty = match[4].toLowerCase();

  if (!Number.isInteger(setNumber) || !scenarioTheme) return normalized;

  return `Set ${toSetLabel(setNumber)} (${difficulty}): ${scenarioTheme}`;
}
