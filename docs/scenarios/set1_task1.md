...
> **Pattern**: 특정 날짜 고정 + 상황 설명 기반 추천 + 프리미엄 좌석 검증
...
2. If several movie options fit, describe the situation and ask AI to recommend a romantic movie for your parents, then follow its recommendation. If that recommendation does not work, follow AI's next-best option.
...
> 기본 리스트에서는 Love Punchline이 첫 번째가 아니므로, 사용자가 상황을 설명하고 추천을 요청하는 상호작용이 영화 선택을 시작하는 자연스러운 진입점이 된다.
...
### 경로 A: 상황 설명 후 추천을 받아 직접 성공

#### [Movie] 1차 방문
- 기본 노출은 Desk for Two → Love Punchline → Autumn Letters
- 부모님 기념일, 로맨틱한 분위기, 실제 기념일 관람이라는 상황을 설명하고 추천을 요청
- AI가 Love Punchline을 가장 자연스러운 후보로 추천
...
> **Conflict C1** (within-stage): P6 "프리미엄 좌석" + P7 "나란히" ↔ 2:15 PM의 남은 프리미엄 좌석 패턴
...
|---|---------|------------|------|-----------|
| C1 | 2:15 PM에는 프리미엄 좌석이 있지만 붙어 있지 않음 | P6+P7 ↔ 좌석 배치 | within-stage | 4:00 PM으로 이동 |
...
- 사용자가 현재 상황을 먼저 설명하고 추천을 받도록 유도할 수 있음
